...
\begin{etaremune}[~{{ rev }}~itemindent=-1.5\bibhang, topsep=0pt,
    ...
    \item ~{{ pub.title }}~ \\
     ~{{ bold_author(pub.authors) }}~. (~{{ pub.year }}~) ~{{ pub.journal }}~ ~{{ pub.numbers }}~
     ~{ if pub.highlights }~
     	\begin{list}{$\drsh$}{}
...
\vspace{0.5cm}
\textit{Total citations (as of last update):} \aiGoogleScholar \hspace{0.05cm} Google Scholar: \textbf{~{{ metrics.gs_citations }}~}
...
\begin{etaremune}[itemindent=-1.5\bibhang, topsep=0pt,
    ...
    \item ~{{ abst.title }}~\\ ~{{ bold_author(abst.authors) }}~. (~{{ abst.year }}~) ~{{ abst.venue }}~. ~{{ abst.nums }}~
     ~{ if abst.highlights }~
     	\begin{innerlist}
